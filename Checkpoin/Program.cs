...
{
    var config = sp.GetRequiredService<IOptions<ConfiguracaoMongoDB>>().Value;
    var livro = sp.GetRequiredService<IMongoClient>();
    var biblioteca = livro.GetDatabase(config.DatabaseName);
    return biblioteca.GetCollection<Livro>(config.BookCollectionName);
});
